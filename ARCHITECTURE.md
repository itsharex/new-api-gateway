...
│   ├── docker-compose.yml     # 生产 compose（基础设施 + 网关 + 分析 worker）
│   └── docker-compose.arm.yml # ARM Mac override（空 override，保留本地开发入口）
...
# 本地开发
make dev -d        # 启动依赖服务（ARM Mac 使用 compose override）
make test          # Go 单元测试
make run           # 启动网关
