...
| `work_relevance.py` | 工作相关性分类器：基于 `context_catalog` aliases/keywords、non-work 规则和 token 成本分层生成 assessment；必要时调用外部 OpenAI-compatible LLM judge，供 `rules.py` 统一转换异常 |
...
工作相关性结果会完整写入 `analysis_results.result_json`；明确非工作相关会直接写入 `usage_anomalies`，未知高成本和工作/非工作冲突则既保留异常记录，也保留 `review` 语义供管理员人工判断。Trace 列表中的 `review` 只对应这些需要人工判断的场景；trace 详情页会额外返回关联异常摘要。
