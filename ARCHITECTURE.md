...
异常菜单与 trace 详情返回的 anomaly 同时包含两套原因字段：`reason` 保留 worker 原始英文原因，`display_reason` 由 admin 端按 anomaly type 生成中文展示文案，供前端列表和详情直接渲染。
...
| `rules.py` | 持久化异常收敛层：统一计算 `effective_tokens = max(prompt_tokens - cached_tokens, 0) + completion_tokens`，并只落库 4 类 anomaly：`high_trace_tokens`、`long_output_anomaly`、`off_hours_high_usage`、`non_work_use` |
...
Trace 列表中的 `needs_review` 只对应 `analysis_results` 里的 review 语义；trace 详情页会额外返回关联 anomaly 摘要，且每条 anomaly 同时带原始 `reason` 与中文 `display_reason`。
