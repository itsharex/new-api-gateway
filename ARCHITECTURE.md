...
| `0014` | 统计基线：`baseline_cache`、`model_artifacts` |
...
| `postgres` | `postgres:16` | 默认启动 | 网关 PostgreSQL（user: `audit`, db: `audit_gateway`） |
...
| `embedding` | `text-embeddings-inference` | `--profile tools` | bge-m3 嵌入服务 |

## 常用命令

```bash
# Docker Compose 部署
docker compose -f deploy/docker-compose.yml --env-file .env.docker.local up -d     # 启动核心服务
docker compose -f deploy/docker-compose.yml --env-file .env.docker.local --profile tools run --rm migrate  # 执行迁移
