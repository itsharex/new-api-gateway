...
4. 工作相关性分类（输出 decision/action/score/evidence；明确非工作相关直接收敛为 `non_work_use`，`work_nonwork_conflict` 仅保留在 `analysis_results` / `needs_review`，unknown 走 `record_only` 不再生成 `unknown_high_cost`）
5. 运行 4 类持久化异常检测：`non_work_use`、`high_trace_tokens`、`long_output_anomaly`、`off_hours_high_usage`
...
管理端异常列表与详情展示会同时返回中文 `display_reason` 和 worker 原始 `reason`，前者用于界面直出，后者保留原始分析字段。
