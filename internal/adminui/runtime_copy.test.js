...
function loadRuntimeCopy() {
  delete require.cache[require.resolve("./runtime_copy.js")];
  return require("./runtime_copy.js");
}

test("runtime_copy does not assign browser global when loaded via Node require", () => {
  delete globalThis.AdminRuntimeCopy;

  const runtimeCopy = loadRuntimeCopy();

  assert.equal(globalThis.AdminRuntimeCopy, undefined);
  assert.equal(typeof runtimeCopy.runtimeSamplingSummary, "function");
});

test("runtimeSamplingRangeLabel maps supported ranges to Chinese labels", () => {
  const { runtimeSamplingRangeLabel } = loadRuntimeCopy();
...
test("runtimeSamplingSummary renders count copy for non-empty history", () => {
  const { runtimeSamplingSummary } = loadRuntimeCopy();
...
test("runtimeSamplingTooltip explains the metric semantics", () => {
  const { runtimeSamplingTooltip } = loadRuntimeCopy();
